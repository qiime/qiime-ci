...
sed -i -e"s?repository-location: git://github.com/picrust/picrust.git?repository-location: $WORKSPACE/picrust\nlocal-repository: yes?" qiime-deploy-conf/picrust-1.0.0-dev/picrust.conf
sed -i -e"s?log-level: INFO?log-level: DEBUG?" qiime-deploy-conf/picrust-1.0.0-dev/picrust.conf
cp -r /home/ubuntu/qiime_software picrust_software
python qiime-deploy/qiime-deploy.py picrust_software -f qiime-deploy-conf/picrust-1.0.0-dev/picrust.conf --force-remove-failed-dirs --force-remove-previous-repos
