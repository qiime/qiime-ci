...
sed -i -e"s?repository-location: git://github.com/qiime/pynast.git?repository-location: $WORKSPACE/pynast\nlocal-repository: yes?" qiime-deploy-conf/pynast-1.2.2-dev/pynast.conf
cp -r /home/ubuntu/qiime_software pynast_software
python qiime-deploy/qiime-deploy.py pynast_software -f qiime-deploy-conf/pynast-1.2.2-dev/pynast.conf --force-remove-failed-dirs --force-remove-previous-repos
